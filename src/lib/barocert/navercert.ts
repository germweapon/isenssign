/**
 * NaverCert (네이버 인증서) 본인인증 및 전자서명 서비스
 *
 * 네이버 인증서를 통해 본인인증 요청을 보내고,
 * 사용자가 네이버 앱에서 인증을 완료하면 결과를 조회합니다.
 *
 * @example
 * ```ts
 * const naverCert = new NaverCert(getBarocertClient());
 * const { receiptID } = await naverCert.requestIdentityVerification(
