/**
 * KakaoCert (카카오 인증서) 본인인증 및 전자서명 서비스
 *
 * 카카오톡 인증서를 통해 본인인증 요청을 보내고,
 * 사용자가 카카오톡에서 인증을 완료하면 결과를 조회합니다.
 *
 * @example
 * ```ts
 * const kakaoCert = new KakaoCert(getBarocertClient());
 * const { receiptID } = await kakaoCert.requestIdentityVerification(
 *   '01012345678', '홍길동', '19900101',
 * );
 * const status = await kakaoCert.getIdentityVerificationStatus(receiptID);
 * if (status.state === 1) {
 *   const result = await kakaoCert.verifyIdentity(receiptID);
 * }
 * ```
 */
